...
class tmdbScreen(Screen, HelpableScreen, CoverHelper):
	...
	def __init__(self, session, text, path="", results=None, keepTemp=False):
		...
		# Tehdään oma varjolista, jotta haku ja indeksit toimivat viiveettä ja kaatumatta
		self.flat_list_data = []
	...
	def tmdbSearch(self):
		...
		if json_data and json_data.get('results'):
			...
			for IDs in json_data['results']:
				...
				elif 'profile_path' in IDs:  # Näyttelijöiden tuki profiilikuville
				...
				if fid or title or media:
					# TÄRKEÄÄ: Ei haeta englanninkielistä tekstiä vielä tässä loopissa (Salama-alkulataus!)
	...
	def getInfo(self):
		...
		# --- LAISKA FALLBACK TÄSSÄ (Haetaan vain sille kohteelle joka on valittu nuolella) ---
		if (not overview_text or not overview_text.strip()) and self.lang != "en":
			try:
				...
				# Päivitetään haettu englanninkielinen teksti talteen välimuistiin
...
class tmdbScreenSeason(Screen, HelpableScreen, CoverHelper):
	...
	def __init__(self, session, mname, fid, media, info):
		...
		# PALAUTETTU ALKUPERÄINEN: Puhdas ScrollLabel ilman listoja tai sinisiä palkkeja!
	...
	def getInfo(self):
		...
		# PALAUTETTU ALKUPERÄINEN: Asetetaan raaka teksti suoraan ScrollLabelille ilman listamuunnosta
	...
	# PALAUTETTU ALKUPERÄINEN: Liikutetaan ScrollLabel-tekstikenttää sivuittain ylös/alas
	def chDown(self):
		self['data'].pageDown()
